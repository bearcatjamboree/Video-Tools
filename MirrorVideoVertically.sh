...
#     video and mirror the video vertically
#
#   USAGE
...
############################
outfile="$name"_mirrored_vertically
newoutfile=$outfile.$ext

ffmpeg -i "$input_file" -vf "crop=iw:ih/2:0:0,split[top][tmp];[tmp]vflip[bottom];[top][bottom] vstack" "$newoutfile"