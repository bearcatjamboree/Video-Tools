...
if [[ "$resolution" == "" ]]; then
    if [[ "$machine" == "Mac" ]]; then
        resolution=$(osascript -e 'set T to text returned of (display dialog "Enter new resolution (1280:720, 1920:1080, 1080:1920, etc.): " buttons {"Cancel", "OK"} default button "OK" default answer "")')
    elif [[ "$machine" == "Linux" ]]; then
        resolution=$(dialog --title "Enter new resolution (1280:720, 1920:1080, 1080:1920, etc.): " --inputbox "resolution:" 8 60)
    elif [[ "$machine" == "Cygwin" ]]; then
        resolution=$(dialog --title "Enter new resolution (1280:720, 1920:1080, 1080:1920, etc.): " --inputbox "resolution:" 8 60)
...
############################
outfile="$name"_border
newoutfile=$outfile.$ext

ffmpeg -i "$input_file" -vf "scale=$resolution:force_original_aspect_ratio=decrease,pad=$resolution:(ow-iw)/2:(oh-ih)/2,setsar=1" -c:a copy "$newoutfile"